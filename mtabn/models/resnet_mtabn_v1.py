...
This network model is originally developed by Hiroshi Fukui,
which is the original implementation published in our CVPR paper.
"""
...
class MtABNResNetV1(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=40, residual_attention=True):
        ...
        self.conv_att = nn.Conv2d(512 * block.expansion, num_classes, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x):
        ...
        out_att = self.global_avg_pool(x_att)  # [batch, num_classes, 1, 1]
...
def mtabn_v1_resnet18(pretrained=False, **kwargs):
    """Multitask Attention Branch Network (V1) with ResNet-18 backbone"""
    model = MtABNResNetV1(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(load_state_dict_from_url(model_urls['resnet18'], progress=True), strict=False)
    return model


def mtabn_v1_resnet34(pretrained=False, **kwargs):
    """Multitask Attention Branch Network (V1) with ResNet-34 backbone"""
    model = MtABNResNetV1(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(load_state_dict_from_url(model_urls['resnet34'], progress=True), strict=False)
    return model


def mtabn_v1_resnet50(pretrained=False, **kwargs):
    """Multitask Attention Branch Network (V1) with ResNet-50 backbone"""
    model = MtABNResNetV1(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(load_state_dict_from_url(model_urls['resnet50'], progress=True), strict=False)
    return model


def mtabn_v1_resnet101(pretrained=False, **kwargs):
    """Multitask Attention Branch Network (V1) with ResNet-101 backbone"""
    model = MtABNResNetV1(Bottleneck, [3, 4, 23, 3], **kwargs)
    if pretrained:
        model.load_state_dict(load_state_dict_from_url(model_urls['resnet101'], progress=True), strict=False)
    return model


def mtabn_v1_resnet152(pretrained=False, **kwargs):
    """Multitask Attention Branch Network (V1) with ResNet-152 backbone"""
    model = MtABNResNetV1(Bottleneck, [3, 8, 36, 3], **kwargs)
    if pretrained:
...
if __name__ == '__main__':
    ...
    model18 = mtabn_v1_resnet18(pretrained=True, num_classes=n_class, residual_attention=True)
    output18 = model18(input)
    print("output of resnet-18", output18[0].size(), output18[1].size(), output18[2].size())

    model34 = mtabn_v1_resnet34(pretrained=True, num_classes=n_class, residual_attention=True)
    output34 = model34(input)
    print("output of resnet-34", output34[0].size(), output34[1].size(), output34[2].size())

    model50 = mtabn_v1_resnet50(pretrained=True, num_classes=n_class, residual_attention=True)
    output50 = model50(input)
    print("output of resnet-50", output50[0].size(), output50[1].size(), output50[2].size())

    model101 = mtabn_v1_resnet101(pretrained=True, num_classes=n_class, residual_attention=True)
    output101 = model101(input)
    print("output1 of resnet-101", output101[0].size(), output101[1].size(), output101[2].size())

    model152 = mtabn_v1_resnet152(pretrained=True, num_classes=n_class, residual_attention=True)
    output152 = model152(input)
    print("output1 of resnet-152", output152[0].size(), output152[1].size(), output152[2].size())
